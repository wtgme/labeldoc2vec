...
try:
    from gensim.models.labeldoc2vec_inner import train_dl_pair, train_document_dbow, train_document_dm, train_document_dm_concat
    ...
    def train_dl_pair(model, label_index, context_index, alpha, learn_vectors=True, learn_hidden=True,
        ...
        neg_size = min(model.negative+1, len(label_vectors))
    ...
    def train_document_dbow(model, doc_words, doctag_indexes, doclabel_indexes, alpha, work=None,
        ...
        for doctag_index in doctag_indexes:
            for doclabel_index in doclabel_indexes:
                train_dl_pair(model, doclabel_index, doctag_index, alpha, learn_vectors=learn_doctags,
                              learn_hidden=learn_hidden, context_vectors=doctag_vectors,
    ...
    def train_document_dm(model, doc_words, doctag_indexes, doclabel_indexes, alpha, work=None, neu1=None,
    ...
    def train_document_dm_concat(model, doc_words, doctag_indexes, doclabel_indexes, alpha, work=None, neu1=None,
                                 learn_doctags=True, learn_words=True, learn_hidden=True,
                                 word_vectors=None, word_locks=None, doctag_vectors=None, doctag_locks=None,
                                 doclabel_vectors=None, doclabel_locks=None):
        """
...
class LabelDoc2Vec(Word2Vec):
    ...
    def _do_train_job(self, job, alpha, inits):
        ...
        for doc in job:
            ...
            if self.sg:
                tally += train_document_dbow(self, doc.words, doctag_indexes, doclabel_indexes, alpha, work,
                ...
                tally += train_document_dm_concat(self, doc.words, doctag_indexes, doclabel_indexes, alpha, work, neu1,
                                                  doctag_vectors=doctag_vectors, doctag_locks=doctag_locks,
                                                  doclabel_vectors=doclabel_vectors, doclabel_locks=doclabel_locks)
            else:
                tally += train_document_dm(self, doc.words, doctag_indexes, doclabel_indexes, alpha, work, neu1,
                                           doctag_vectors=doctag_vectors, doctag_locks=doctag_locks,
                                           doclabel_vectors=doclabel_vectors, doclabel_locks=doclabel_locks)
    ...
    def infer_vector(self, doc_words, alpha=0.1, min_alpha=0.0001, steps=5):
        ...
        for i in range(steps):
            if self.sg:
                train_document_dbow(self, doc_words, doctag_indexes, doclabel_indexes, alpha, work,
                                    learn_words=False, learn_hidden=False,
                                    doctag_vectors=doctag_vectors, doctag_locks=doctag_locks,
                                    doclabel_vectors=doclabel_vectors, doclabel_locks=doclabel_locks)
            elif self.dm_concat:
                train_document_dm_concat(self, doc_words, doctag_indexes, doclabel_indexes, alpha, work, neu1,
                                         learn_words=False, learn_hidden=False,
                                         doctag_vectors=doctag_vectors, doctag_locks=doctag_locks,
                                         doclabel_vectors=doclabel_vectors, doclabel_locks=doclabel_locks)
            else:
                train_document_dm(self, doc_words, doctag_indexes, doclabel_indexes, alpha, work, neu1,
                                  learn_words=False, learn_hidden=False,
                                  doctag_vectors=doctag_vectors, doctag_locks=doctag_locks,
                                  doclabel_vectors=doclabel_vectors, doclabel_locks=doclabel_locks)
