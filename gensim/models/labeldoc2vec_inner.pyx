...
print '-----------------------------------------------'
print '---------LabelDoc2Vec Faster Version-----------'
print '-----------------------------------------------'

cdef void printf_list(int size, REAL_t *context_vectors) nogil:
    for i in range(size):
        printf('%f ', &context_vectors[i])
    printf("\n");
...
cdef unsigned long long train_dl_pair(
    const int negative, const int size, const int table_len, REAL_t *work, const int neg_size, np.uint32_t *cum_table,
    unsigned long long next_random, const np.uint32_t word_index,
    const np.uint32_t context_index, const REAL_t alpha, int learn_context, int learn_hidden,
    REAL_t *context_vectors, REAL_t *context_locks, REAL_t *label_vectors,
    REAL_t *label_locks) nogil:
    ...
    for d in range(neg_size+1):
        if d == 0:
            target_index = word_index
            label = ONEF
        else:
            target_index = bisect_left(cum_table, (next_random >> 16) % table_len, 0, neg_size-1)
            next_random = (next_random * <unsigned long long>25214903917ULL + 11) & modulo
            if target_index == word_index:
                continue
            label = <REAL_t>0.0
        #printf('%d %f %d \n', word_index, label, target_index)
        row2 = target_index * size
        #printf('row2 %f \n', row2)
        f = our_dot(&size, &context_vectors[row1], &ONE, &label_vectors[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
        #printf('\n--------------lable-tag----g------%f \n', g)
        our_saxpy(&size, &g, &label_vectors[row2], &ONE, work, &ONE)
        if learn_hidden:
            our_saxpy(&size, &g, &context_vectors[row1], &ONE, &label_vectors[row2], &ONE)
    if learn_context:
        our_saxpy(&size, &context_locks[context_index], work, &ONE, &context_vectors[row1], &ONE)
    #printf('\n----------------------------------\n')
    #printf_list(size, &label_vectors[0]) 
    return next_random
...
cdef unsigned long long fast_document_dbow_neg(
    ...
    if learn_context:
        our_saxpy(&size, &context_locks[context_index], work, &ONE, &context_vectors[row1], &ONE)
    #printf('\n-------------DOCMENT---------------------\n')
    #printf_list(size, &context_locks[0])
    return next_random
...
def train_label_document_dbow(model, doc_words, doctag_indexes, doclabel_indexes, alpha, work=None,
    ...
    cdef REAL_t *_doclabel_vectors
    cdef REAL_t *_word_locks
    cdef REAL_t *_doctag_locks
    cdef REAL_t *_doclabel_locks
    cdef REAL_t *_work
    ...
    cdef np.uint32_t _doclabel_indexes[MAX_DOCUMENT_LEN]
    cdef np.uint32_t reduced_windows[MAX_DOCUMENT_LEN]
    cdef int document_len
    cdef int doctag_len
    cdef int doclabel_len
    cdef int label_table_len
    ...
    _doclabel_vectors = <REAL_t *>(np.PyArray_DATA(doclabel_vectors))
    label_table_len = len(doclabel_vectors)

    # Negative sampling for Label vectors
    cdef int neg_size_label
    cdef np.uint32_t *cum_table_label
    neg_size_label = <int>min((negative+1), label_table_len)
    cum_table_label = <np.uint32_t *>(np.PyArray_DATA(np.array(range(neg_size_label))))
    ...
    _doclabel_locks = <REAL_t *>(np.PyArray_DATA(doclabel_locks))
    ...
    doclabel_len = <int>min(MAX_DOCUMENT_LEN, len(doclabel_indexes))
    for i in range(doctag_len):
        _doctag_indexes[i] = doctag_indexes[i]
        result += 1
    for i in range(doclabel_len):
        _doclabel_indexes[i] = doclabel_indexes[i]
    ...
    with nogil:
        for i in range(document_len):
            ...
            for j in range(doctag_len):
                ...
                if doclabel_len > 0:
                    for t in range(doclabel_len):
                        next_random = train_dl_pair(negative, size, label_table_len, _work, neg_size_label, cum_table_label,
                        next_random, _doclabel_indexes[t], _doctag_indexes[j], _alpha,
                        _learn_doctags, _learn_hidden, _doctag_vectors, _doctag_locks, _doclabel_vectors, _doclabel_locks)
    return result
...
def train_label_document_dm(model, doc_words, doctag_indexes, doclabel_indexes, alpha, work=None, neu1=None,
    ...
    with nogil:
        for i in range(document_len):
            ...
            if doclabel_len > 0:
                for p in range(doctag_len):
                    for q in range(doclabel_len):
                        #printf('%f %f %f %f \n', i, j, _doctag_indexes[i], _doclabel_indexes[j])
                        next_random = train_dl_pair(negative, size, label_table_len, _work, neg_size_label,
                        cum_table_label, next_random, _doclabel_indexes[q], _doctag_indexes[p], _alpha,
                        _learn_doctags, _learn_hidden, _doctag_vectors, _doctag_locks, _doclabel_vectors, _doclabel_locks)
    return result
...
def train_label_document_dm_concat(model, doc_words, doctag_indexes, doclabel_indexes, alpha, work=None, neu1=None,
    ...
    _doctag_vectors = <REAL_t *>(np.PyArray_DATA(doctag_vectors))
    if doclabel_vectors is None:
       doclabel_vectors = model.labelvecs.doclabel_syn0
    _doclabel_vectors = <REAL_t *>(np.PyArray_DATA(doclabel_vectors))
    label_table_len = len(doclabel_vectors)
    ...
    _doctag_locks = <REAL_t *>(np.PyArray_DATA(doctag_locks))
    if doclabel_locks is None:
       doclabel_locks = model.labelvecs.doclabel_syn0_lockf
    _doclabel_locks = <REAL_t *>(np.PyArray_DATA(doclabel_locks))
    ...
    with nogil:
        for i in range(document_len):
            ...
            if doclabel_len > 0:
                for p in range(doctag_len):
                    for q in range(doclabel_len):
                        next_random = train_dl_pair(negative, vector_size, label_table_len, _work, neg_size_label,
                        cum_table_label, next_random, _doclabel_indexes[q], _doctag_indexes[p], _alpha,
                        _learn_doctags, _learn_hidden, _doctag_vectors, _doctag_locks, _doclabel_vectors, _doclabel_locks)
    return result
